...
def _build_paradata_config(args, config: _cp.ConfigParser) -> dict:
    ...
    return {
        ...
        "vocabulary":    str(getattr(args, "vocabulary", "") or ""),
        "chunk_limit":   4000,
...
def fetch_xml_from_url(url, download_dir):
    ...
    try:
        ...
        safe_name = "".join(
            c for c in doc_id if c.isalpha() or c.isdigit() or c in ("-", "_")
        ).rstrip()
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of allowed file formats",
    )
    ...
    if args.config and args.config.exists():
        ...
        if args.source_lang == "cs" and "source_lang" in defaults:
            args.source_lang = defaults["source_lang"]
        if args.target_lang == "en" and "target_lang" in defaults:
            args.target_lang = defaults["target_lang"]
        if args.formats is None and "formats" in defaults:
            args.formats = defaults["formats"]
        ...
        if args.vocabulary is None and "vocabulary" in defaults:
            ...
            args.vocabulary = vocab_candidate if vocab_candidate.exists() else None
...
def main():
    ...
    if not input_path or (not input_path.is_dir() and not input_path.is_file()):
        print("[ERROR] Input path does not exist. Please provide a valid file or directory.")
        return
    ...
    # Build translator – pass vocabulary path when available
    vocab_path = getattr(args, "vocabulary", None)
    translator = LindatTranslator(vocab_path=vocab_path)

    # Initialise FastText only when 'auto' detection is requested
        ...
        for i, file_path in enumerate(files_to_process, 1):
            ...
            with open(csv_path, "w", encoding="utf-8", newline="") as csv_file:
                ...
                try:
                    if args.alto:
                        process_alto_xml(
                            file_path, output_file, translator,
                            args.source_lang, args.target_lang,
                            csv_writer, identifier,
                        )
                    else:
                        process_amcr_xml(
                            file_path, output_file, xpaths_list, translator,
                            args.source_lang, args.target_lang,
                            args.xsd, csv_writer, identifier,
                        )
